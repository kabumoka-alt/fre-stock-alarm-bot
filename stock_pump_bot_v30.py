...
MAX_BUY_PER_SCAN      = 2    # [v35.1] 랭킹 후 스캔 1회당 신규 매수 최대 종목 수
MAX_POSITIONS         = 4    # [v31] 7 → 4 (자본 집중)
MIN_ENTRY_SCORE       = 70.0
NEW_BUY_COOLDOWN_SEC  = 60
...
# [v35.1] 마지막 랭킹 진입 시각. 상태 파일에 저장해 재시작 후에도 쿨다운 유지.
last_new_buy_time = None

V35_CSV_FIELDS = [
    "timestamp_et", "scan_id", "symbol", "score", "momentum_1m_pct",
    "momentum_5m_pct", "dollar_volume_5m", "volume_multiplier", "fading",
    "existing_filter_passed", "selected_for_buy", "skip_reason", "entry_price",
    "final_exit_price", "final_pnl_pct", "exit_reason",
]
...
def save_sim_state():
    ...
    payload = {
        ...
        "last_new_buy_time": (
            last_new_buy_time.isoformat() if isinstance(last_new_buy_time, datetime) else None
        ),
    }
    try:
...
def reset_sim_state_in_memory():
    ...
    global daily_start_pnl, daily_start_cash, last_new_buy_time
    sim_positions.clear()
    entry_prices.clear()
    trade_log.clear()
...
def load_sim_state() -> bool:
    ...
    try:
        ...
        last_new_buy_time = _parse_datetime(payload.get("last_new_buy_time"))
...
def sim_close(sym: str, exit_price: float, reason: str, qty: int = None) -> str:
    ...
    position_scan_id = pos.get("scan_id", "")
...
def score_candidate(symbol: str, bars: list, current_price: float) -> dict:
    ...
    if not bars or len(bars) < 6:
        return {"score": None, "reason": "insufficient_bars"}
    try:
        recent = bars[-5:]
        ...
        if current_price <= 0 or closes[0] <= 0 or closes[-2] <= 0 or not prior_volumes:
            return {"score": None, "reason": "invalid_score_data"}
        ...
        return {"score": None, "reason": f"score_error:{type(e).__name__}"}


def rank_candidates(candidates: list) -> list:
    """점수, 최근 5분 거래대금, 최근 1분 상승률 순으로 안정 정렬."""
    return sorted(
        candidates,
        key=lambda item: (
            item["score_data"]["score"],
            item["score_data"]["dollar_volume_5m"],
            item["score_data"]["momentum_1m_pct"],
        ),
        reverse=True,
    )
...
def run_scan():
    ...
    # [v35.1] 직전 스캔 배치의 마지막 신규매수부터 60초간 다음 배치를 차단.
    if last_new_buy_time is not None:
        cooldown_elapsed = (now_utc - last_new_buy_time).total_seconds()
        if cooldown_elapsed < NEW_BUY_COOLDOWN_SEC:
            remaining = NEW_BUY_COOLDOWN_SEC - cooldown_elapsed
            print(f"  [⏳ 신규매수 쿨다운] {remaining:.0f}초 남음 — 보유분 감시는 계속")
            csv_rows.extend(
                _candidate_csv_row(
                    scan_id, stock["symbol"], skip_reason="buy_cooldown"
                ) for stock in top
            )
    ...
    for stock in top:
        ...
        if sym in blacklisted_today:
            csv_rows.append(_candidate_csv_row(
                scan_id, sym, skip_reason="blacklisted"
            ))
            continue
        bars = get_bars(sym)
    ...
    for stock in top_with_atr:
        ...
        if sym in last_alert:
            ...
            if elapsed < COOLDOWN_MINUTES:
                csv_rows.append(_candidate_csv_row(
                    scan_id, sym, skip_reason="candidate_cooldown"
                ))
                continue
        ...
        if result is None:
            csv_rows.append(_candidate_csv_row(
                scan_id, sym, existing_filter_passed=False,
                skip_reason="existing_filter_failed",
            ))
            continue

        score_data = score_candidate(sym, stock.get("_bars"), stock["price"])
        if score_data.get("score") is None:
            reason = score_data.get("reason", "score_failed")
            print(f"  [점수 실패] {sym} — {reason}")
            csv_rows.append(_candidate_csv_row(
                scan_id, sym, score_data, existing_filter_passed=True,
                skip_reason=reason,
            ))
            continue
    ...
    if ranked_candidates:
        print("  [v35.1 후보 랭킹] " + " | ".join(
            f"{item['symbol']}({item['score_data']['score']:.1f})"
    ...
    for stock in ranked_candidates:
        ...
        with state_lock:
            ...
            last_new_buy_time  = now_utc
            sim_positions[sym]["scan_id"] = scan_id
        ...
        print(
            f"[🚀 감지] {sym} | 점수 {score:.1f} | {stock['change_pct']:+.2f}% | RSI {result['rsi']:.1f} "
...
def main():
    ...
    print(f"🧮 [v35.1] 후보 점수 내림차순 | 최소 {MIN_ENTRY_SCORE:.0f}점 | 신규매수 쿨다운 {NEW_BUY_COOLDOWN_SEC}초")
    ...
    send_telegram(
        f"🤖 <b>급등 감지 내부 시뮬레이션 봇 v35.1 시작!</b>\n"
        ...
        f"🩸 <b>[v35.1] 청산 판단 = 유효한 호가(bid) 기준</b> (stale 가격 청산 차단)\n"
        f"⚖️ <b>[v35.1] ATR 사이징·포지션 회계 강화</b> — 1건 리스크 ≤{MAX_RISK_PER_TRADE_PCT}% | 종목당 ≤{MAX_POSITION_PCT}%\n"
        f"💾 재시작 시 가상 현금·포지션·손익 자동 복원\n"
        f"⏳ [v35.1] 개장 {OPEN_BLACKOUT_MIN}분 신규진입 금지 | 최소 {MIN_QTY}주\n"
