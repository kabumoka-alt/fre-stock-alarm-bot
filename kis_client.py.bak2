...
def place_domestic_order(code: str, qty: int, price: int, side: str) -> dict:
    """
    국내주식 주문 (현금매수/매도).
    code: 6자리 종목코드 (예: "005930" 삼성전자)
    price: 원 단위 지정가 (시장가는 KIS 정책상 ORD_DVSN/가격 처리 별도 확인 필요)
    side: "buy" 또는 "sell"
    """
    if side not in ("buy", "sell"):
        raise ValueError("side는 'buy' 또는 'sell'이어야 합니다")
    ...
    result = resp.json()
    if result.get("rt_cd") != "0":
        print(f"[KIS 국내주문 오류] {code} {side} → {result}")
    else:
        print(f"[KIS 국내주문 성공] {code} {side} {qty}주 @ {price}원 → {result.get('msg1')}")
    return result
